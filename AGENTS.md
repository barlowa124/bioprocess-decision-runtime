...
- Limit universal-proof claims to the exact SMT formulas, widths, domains, and explicit premises recorded in each obligation. Describe CUDA manifests as launch and binary provenance and SASS equations as proposed subset semantics, not hardware or instruction-level verification. Distinct-kernel suite coverage means one attested invocation per recorded symbol, not every invocation.
