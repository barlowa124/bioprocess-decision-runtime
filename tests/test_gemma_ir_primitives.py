...
class GemmaIrPrimitiveTests(unittest.TestCase):
    ...
    def test_gate_blocks_unqualified_floating_point_primitives(self) -> None:
        ...
        gate = build_primitive_qualification_gate(self.program, self.certificate)
        verification = verify_primitive_qualification_gate(
            self.program, self.certificate, gate
        )
        self.assertTrue(verification["valid"], verification)
        self.assertEqual(verification["independently_tested_opcode_count"], 8)
        self.assertEqual(verification["unresolved_floating_point_opcode_count"], 11)
        ...
        self.assertIn("LINEAR", gate["unresolved_floating_point_opcodes"])
        self.assertIn("SOFTMAX", gate["unresolved_floating_point_opcodes"])
    ...
    def test_gate_rejects_forged_activation(self) -> None:
        ...
        verification = verify_primitive_qualification_gate(
            self.program, self.certificate, damaged
        )
        self.assertFalse(verification["valid"])
