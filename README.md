...
The linear baseline records 364 memory-address slices, 123 with at least one parameter-field text dependency. A fixed-point direct-branch CFG contains 162 basic blocks and 3,653 edges; 133 blocks and 3,386 instructions are reachable under the modeled edges. It resolves all 99 direct branch targets, includes both target and fallthrough edges for 12 direct calls, encounters three returns, and converges after 3,850 block iterations. The CFG analysis records 358 reachable memory-address slices, 126 with parameter-field dependencies. Every target pointer field reaches at least one address operand in both analyses.

This remains a syntactic over-approximation. Predicated definitions merge executed and non-executed possibilities, but predicate truth is not modeled. Call targets and fallthroughs are included without matching returns to call sites; 50 `BSSY`, `BSYNC`, `BREAK`, or `BRX` reconvergence/control instructions remain unmodeled. Instruction behavior, field-specific load-versus-store direction, bounds, aliasing, and hardware execution are also outside scope. Accordingly, `direct_branch_cfg_reaching_definitions_established` is true while `call_return_dataflow_established`, `reconvergence_dataflow_established`, `predicate_truth_modeled`, `complete_control_flow_dataflow_established`, `memory_access_direction_established`, `memory_bounds_established`, `sass_instruction_semantics_established`, and `hardware_conformance_established` remain false.
