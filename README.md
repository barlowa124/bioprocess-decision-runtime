...
scoped to what the evidence shows. The
...
- The same compiler covers standard decoder families (Qwen-2/Llama-style) via declared-opcode extensions: `RMS_NORM_PLAIN` for non-offset normalization weights, `LINEAR_BIAS` for biased projections, `SILU` for SwiGLU activation, residual-then-norm layer layout when feedforward sandwich norms are absent, `head_dim` derived from `hidden_size / heads` when unconfigured, `use_sliding_window: false` honored over a present window value, and no embedding scale. Extended-family programs declare the full semantics table while legacy-only programs keep the original 20-entry set, so a recompiled pinned-270m program reproduces its frozen `program_sha256` byte-identically. Unknown families, unknown `hidden_act` values, non-linear rope scaling, and inconsistent norm layouts reject rather than compile an unverified structure
...
Open the local address printed by the command. Port `0` selects an available port. `--port 8765` requests a fixed port if it is free. `--root` can select a repository containing the saved `results/` artifacts and demonstration policy. This uses the Python standard library and locally served HTML/CSS/JavaScript, so no frontend installation, external assets, or cloud inference calls are needed.
...
Both complete predictions must be frozen before any of the six held-out native forwards. An abstention preserves both declared case outcomes and blocks every native call. Replay recomputes and freezes both predictions before any replay forward. Mismatches are retained, never refitted. `--resume` requires an existing compatible run and explicitly discloses restored predictions/native reports. A completed aggregate output is never overwritten. `plan` and `compare` can run the stages separately.
